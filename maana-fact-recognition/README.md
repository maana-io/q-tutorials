...
The various queries can be used as part of function composition, so the output of one query can be used as the input of another.  The examples below show basic use inside the function graph.  In each case, use the service inventory for Maana Fact Recognition and drag the functions to the knowledge graph.  Next, press the arrow button on the right hand panel to bring up the function input window.

Drag "extractTriples" onto the graph.
...
Drag "extractByPattern" onto the graph.
...
Drag "extractByExample" onto the graph.
